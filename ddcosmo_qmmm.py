...
# The factor numpy.sqrt(4*numpy.pi) is due to the product of 4*pi * Y_0^0
    ...
    xi_jn = numpy.einsum('n,jn,xn,jx->jn', weights_1sph, ui[:natm], ylm_1sph, L_S)
    extern_point_idx = ui[:natm] > 0
    cav_coords = (mol.atom_coords().reshape(natm,1,3)
                  + numpy.einsum('r,gx->rgx', r_vdw[:natm], coords_1sph))
    cav_coords = cav_coords[extern_point_idx]
...
class DDCOSMO(lib.StreamObject):
    def __init__(self, mol, mm_mol=None):
        ...
        self.eps = 78.3553
        self.grids = gen_grid.Grids(mol) # Generate grids of QM atoms, but no of MM atoms
